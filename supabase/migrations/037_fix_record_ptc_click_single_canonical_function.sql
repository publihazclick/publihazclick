-- =============================================================================
-- Migration 037: Una sola función canónica record_ptc_click
-- - Todos los params TEXT (sin VARCHAR) → sin ambigüedad en PostgREST
-- - p_ad_type_override opcional (7mo param) para mini_referral
-- - Mega: límite dinámico = 5 × afiliados_activos (máx 200), reset diario
-- - Mantiene bonus de referido en standard_400
-- =============================================================================

-- Eliminar todas las versiones anteriores
DROP FUNCTION IF EXISTS record_ptc_click(UUID, UUID, TEXT,    TEXT, TEXT,    INTEGER);
DROP FUNCTION IF EXISTS record_ptc_click(UUID, UUID, VARCHAR, TEXT, VARCHAR, INTEGER);
DROP FUNCTION IF EXISTS record_ptc_click(UUID, UUID, VARCHAR, TEXT, VARCHAR, INTEGER, VARCHAR);
DROP FUNCTION IF EXISTS record_ptc_click(UUID, UUID, TEXT,    TEXT, TEXT,    INTEGER, TEXT);

-- Crear la función canónica única
...
BEGIN
  ...
  -- Tipo efectivo: override (mini_referral) o el tipo real del anuncio
  v_effective_type := COALESCE(p_ad_type_override, v_task.ad_type::text);

  -- 2. Dedup: ya clickeó este anuncio HOY
  IF v_effective_type = 'mini_referral' THEN
    -- mini_referral: verificar en daily_task_assignments
    ...
    END IF;
    -- Verificar que el task fue asignado como mini_referral
    SELECT COUNT(*) INTO v_already_done
    FROM daily_task_assignments
    WHERE user_id        = p_user_id
      AND task_id        = p_task_id
    ...
    END IF;
  ELSE
    -- Resto de tipos: dedup por ptc_clicks del día
    SELECT COUNT(*) INTO v_already_done
    FROM ptc_clicks
    WHERE user_id = p_user_id
      AND task_id = p_task_id
  ...
  -- 3. Calcular límite diario por tipo efectivo
  IF v_effective_type = 'mega' THEN
    -- Mega: 5 slots por afiliado activo, máximo 200 (40 afiliados × 5)
  ...
  ELSE
    v_daily_limit := CASE v_effective_type
      ...
      WHEN 'mini_referral' THEN 200  -- controlado por slots de asignación
      ELSE 5
    END;
  END IF;
  ...
  -- 5. Recompensa fija por tipo efectivo
  ...
  -- 6. Registrar click con metadata anti-fraude
  ...
  -- 8. Marcar asignación diaria como completada
