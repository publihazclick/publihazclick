-- Migration 070: Renovar paquete con saldo acumulado (real_balance)
-- Transacción atómica: descuenta saldo, crea user_package, actualiza perfil
...
BEGIN
  ...
  -- Si tiene paquete activo del MISMO tipo y no ha expirado, extender desde la fecha actual de expiración
  ...
  -- 8. Marcar paquetes anteriores del mismo usuario como expirados (excepto el nuevo)
  ...
  -- 9. Registrar en payments para auditoría
  INSERT INTO payments (user_id, package_id, amount, currency, status, payment_method, gateway_transaction_id)
  VALUES (p_user_id, p_package_id, v_price_cop, 'COP', 'approved', 'balance', 'BALANCE-' || v_up_id::TEXT);
