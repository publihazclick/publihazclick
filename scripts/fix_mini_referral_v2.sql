-- Mini referral slots solo para referidos V1 (registrados antes del 10 abril 2026)
-- Los referidos V2 solo dan mega grants por compra, nada más
...
BEGIN
  ...
  -- Contar TODOS los referidos activos (para std400 limit)
  SELECT COUNT(*)::INT INTO v_active_refs
  FROM profiles
  WHERE referred_by = p_user_id AND has_active_package = true;

  -- Contar solo referidos V1 activos (para mini referral y comisiones)
  SELECT COUNT(*)::INT INTO v_active_refs_v1
  FROM profiles
  WHERE referred_by = p_user_id AND has_active_package = true
  ...
  -- Mini referral: solo por referidos V1
  v_mini_ref_slots := v_active_refs_v1 * get_mini_referral_slots_per_affiliate(v_active_refs_v1);
  ...
  RETURN jsonb_build_object(
    ...
    'active_refs_v1', v_active_refs_v1,
