...
export class AdvertiserTasksComponent implements OnInit, OnDestroy {
  ...
  private async checkRequirements(): Promise<void> {
    ...
    if (!user) return;
    // Fuente de verdad: get_user_ad_limits mira package_started_at del perfil,
    // así cada compra/renovación de paquete reinicia el gate automáticamente.
    const { data } = await this.supabase.rpc('get_user_ad_limits', { p_user_id: user.id });
    this.userHasAd.set(!!data?.has_ptc_ad);
    this.userHasBanner.set(!!data?.has_banner_ad);
  }
