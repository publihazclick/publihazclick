            ...
            @for (item of agMenuItems; track item.label) {
              @if (item.divider) {
                ...
                <button (click)="openPassengerSection(item.action)"
                  ...
                  [style.color]="item.icon === 'whatsapp' ? '#25d366' : '#cbd5e1'"
                  [style.background]="item.icon === 'whatsapp' ? 'rgba(37,211,102,0.07)' : 'transparent'"
                  onmouseover="this.style.background=this.getAttribute('data-wsp')==='1'?'rgba(37,211,102,0.15)':'rgba(249,115,22,0.08)'"
                  onmouseout="this.style.background=this.getAttribute('data-wsp')==='1'?'rgba(37,211,102,0.07)':'transparent'"
                  [attr.data-wsp]="item.icon === 'whatsapp' ? '1' : null">
                  @if (item.icon === 'whatsapp') {
                    <svg class="flex-shrink-0" width="20" height="20" viewBox="0 0 24 24" fill="#25d366">
            ...
            @for (item of driverMenuItems; track item.label) {
              @if (item.divider) {
                ...
                <button (click)="openDriverSection(item.action)"
                  class="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-left transition-all active:scale-[0.98] mb-0.5"
                  [style.color]="item.icon === 'whatsapp' ? '#25d366' : item.danger ? '#f87171' : '#cbd5e1'"
                  [style.background]="item.icon === 'whatsapp' ? 'rgba(37,211,102,0.07)' : 'transparent'"
                  onmouseover="this.style.background=this.getAttribute('data-wsp')==='1'?'rgba(37,211,102,0.15)':'rgba(8,145,178,0.08)'"
                  onmouseout="this.style.background=this.getAttribute('data-wsp')==='1'?'rgba(37,211,102,0.07)':'transparent'"
                  [attr.data-wsp]="item.icon === 'whatsapp' ? '1' : null">
                  @if (item.icon === 'whatsapp') {
                    <svg class="flex-shrink-0" width="20" height="20" viewBox="0 0 24 24" fill="#25d366">
...
export class AndaGanaComponent implements OnInit, OnDestroy {
  ...
  readonly agMenuItems = [
    ...
    { divider: true,  section: 'Soporte directo', icon: '', label: '', action: '' },
    { icon: 'whatsapp',         label: 'Soporte por WhatsApp',     action: 'whatsapp_support',  divider: false, section: '' },
  ];
  ...
  async ngOnInit() {
    ...
    if (this.route.snapshot.queryParamMap.get('wallet') === 'result') {
      ...
      if (isPlatformBrowser(this.platformId)) {
        window.history.replaceState({}, '', '/anda-gana');
      }
    }
  ...
  private async _initDriverHome(mine: any) {
    ...
    const _checkingWalletResult = this.walletPaymentResult() === 'processing';
    this.agService.getDriverWalletBalance(mine.id).then(balance => {
      if (balance !== null) {
        this.driverWalletBalance.set(balance);
      }
      if (_checkingWalletResult) {
        const preBalance = parseFloat(localStorage.getItem('movi-pre-recharge-balance') ?? '-1');
        localStorage.removeItem('movi-pre-recharge-balance');
...
<p style="margin-top:30px;font-size:12px;color:#666">Este recibo fue generado automáticamente por Movi (Publihazclick). Para facturación electrónica, contacta soporte.</p>
  ...
  async startWalletRecharge(): Promise<void> {
    ...
    try {
      ...
      // Guardar saldo actual para detectar si el pago realmente se procesó al volver
      const currentBalance = this.driverWalletBalance();
      if (currentBalance !== null) {
        localStorage.setItem('movi-pre-recharge-balance', String(currentBalance));
      }
      handler.open({
        name:         params['name'],
